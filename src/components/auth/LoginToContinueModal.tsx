...
/**
 * Dismissible prompt to sign in (e.g. Song Wars join/submit). Unlike IdentityRequiredModal,
 * the rest of the page stays usable when closed.
 */
export function LoginToContinueModal({
  open,
  onOpenChange,
  title = "Sign in to continue",
  description = "Create an account or log in to join Song Wars and submit tracks.",
}: Props) {
  const pathname = usePathname() || "/app/songwars";
      ...
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            >
              <Link href={`/signup?next=${next}`} onClick={() => onOpenChange(false)}>
                Sign Up
              </Link>
            </Button>
            <Button
            ...
            >
              <Link href={`/login?next=${next}`} onClick={() => onOpenChange(false)}>
                Log In
              </Link>
            </Button>
          </div>
